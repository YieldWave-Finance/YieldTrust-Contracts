...
fn test_prune_finalized_grant_success() {
    let env = Env::default();
    env.mock_all_auths();
    let (_admin, grant_token_addr, _treasury, _oracle, native_token_addr, client) =
        setup_test(&env);
    ...
    let stored_tombstone: Bytes = env.as_contract(&contract_id, || {
        env.storage()
            .persistent()
            .get(&StorageKey::Tombstone(grant_id))
            .expect("Tombstone should exist")
    });
    assert_eq!(
        stored_tombstone,
        expected_hash.into(),
        "Tombstone hash should match"
    );

    // Relayer should receive bounty
    assert_eq!(
        native_token.balance(&relayer),
        200_000,
        "Relayer should receive 0.02 XLM bounty"
    );

    // Grant ID should be removed from recipient's list
    assert!(
        !client.is_active_grantee(&recipient),
        "Recipient should no longer have active grants"
    );
}
