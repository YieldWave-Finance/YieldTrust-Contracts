...
impl MicroStreamTestState {
    ...
    fn setup_micro_streams(
        ...
        let total_amount_per_stream =
            MICRO_STREAM_RATE * SECONDS_PER_DAY as i128 * TEST_DURATION_DAYS as i128;
    ...
    fn calculate_theoretical_distribution(&self, elapsed_seconds: u64) -> (i128, i128) {
        let base_distribution =
            MICRO_STREAM_RATE * elapsed_seconds as i128 * NUM_MICRO_STREAMS as i128;
    ...
    fn verify_rounding_invariants(
        ...
        let (theoretical_grantee, theoretical_validator) =
            self.calculate_theoretical_distribution(elapsed_seconds);
        ...
        if let Some(validator) = self.client.get_grant(1).validator {
            ...
            if (total_accounted_with_validator - self.initial_contract_balance).abs()
                > rounding_tolerance
            {
                return Err(format!(
...
fn test_maximum_micro_streams_stress() {
    let mut state = MicroStreamTestState::new();
    state
        .setup_micro_streams(true)
        .expect("Failed to setup micro streams");
    ...
    state
        .verify_rounding_invariants(year_seconds, true)
...
fn test_dust_accumulation_and_treasury_return() {
    ...
    state
        .simulate_withdrawals(remaining_withdrawals)
        .expect("Withdrawal simulation failed");
    ...
    assert!(
        contract_balance_after <= max_dust,
        "Too much dust remaining in contract: {}",
    ...
    assert!(
        (total_accounted - expected_total).abs() <= tolerance,
...
fn test_rounding_error_mathematical_bounds() {
    ...
    for days in test_periods {
        ...
        let (theoretical_grantee, theoretical_validator) =
            state.calculate_theoretical_distribution(elapsed_seconds);
        ...
        let max_error_per_stream =
            (MICRO_STREAM_RATE * SECONDS_PER_DAY as i128) / 10000;
        ...
        state
            .setup_micro_streams(true)
            .expect("Failed to reset micro streams");
    }
}
...
fn test_single_stroop_precision_edge_case() {
    ...
    let total_per_stream =
        ultra_micro_rate * SECONDS_PER_DAY as i128 * test_duration_days as i128;
    ...
    let expected_remaining =
        total_all_streams - (ultra_micro_rate * test_duration_days as i128 * 1000);
