...
vote to rotate the contract logic to a new implementation (bug fixes, new asset
support, etc.) while guaranteeing that the **immutable terms** of every active
grant stream — funder address, recipient address, and total grant amount — can
never be altered by any upgrade.
...
Active grant streams have no upgrade path. A critical security patch or new
feature (e.g. new asset support) would require migrating 1,000+ live streams,
breaking continuity and trust.
...
- Immutable terms live in the proxy's own storage (unreachable by delegatecall'd
  logic)
...
If the new logic disagrees with even one sampled grant's terms, the entire
upgrade reverts.
...
4. Anyone calls `execute()` after deadline — proxy rotates if quorum + majority
   met
5. `LogicUpgraded(newImpl, logicHash, proposer)` emitted for off-chain
   auditability
...
- **Spot-check is caller-supplied but DAO-accountable** — DAO members are
  incentivised to supply a representative sample; full verification can be done
...
- [ ] `upgradeLogic()` reverts if any sampled grant fails
      `verifyImmutableTerms()`
