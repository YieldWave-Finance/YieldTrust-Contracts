...
This guide explains how Grant-Stream integrates with Stellar's native identity
and authentication standards, enabling seamless wallet integration and ecosystem
...
SEP-10 provides a standardized way for Stellar accounts to authenticate with web
services using cryptographic signatures.

### SEP-12: Stellar Account Verification

SEP-12 defines a standard for KYC/AML verification processes and status sharing
...
async function requestChallenge(userPublicKey, clientDomain) {
  const response = await fetch(
    'https://grant-stream-api.example.com/challenge',
    {
      method: 'POST',
      headers: {
...
async function submitKYCStatus(accountAddress, status, providerSignature) {
  const response = await fetch(
    'https://grant-stream-api.example.com/kyc/submit',
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
async function verifyKYCStatus(accountAddress) {
  const response = await fetch(
    `https://grant-stream-api.example.com/kyc/${accountAddress}`,
  );
...
describe('SEP-10 Integration', () => {
  ...
  test('should reject expired challenges', async () => {
    ...
    await expect(authenticate(signedChallenge)).rejects.toThrow(
      'Challenge expired',
    );
  });
});
...
describe('SEP-12 Integration', () => {
  test('should submit KYC status', async () => {
    const result = await submitKYCStatus(
      testAccount,
      'VERIFIED',
      providerSignature,
    );
  ...
  test('should reject invalid KYC provider', async () => {
    await expect(
      submitKYCStatus(testAccount, 'VERIFIED', invalidSignature),
...
- GitHub:
  [Grant-Stream Issues](https://github.com/Grant-Stream/contracts/issues)
