...
The Dispute Resolution Arbitration Escrow system provides a comprehensive Web3
courtroom for high-stakes grants, ensuring that social friction is resolved
through a transparent and fair legal process. When a DAO claims a project was
never delivered, funds are moved to a neutral "Jury" (Escrow Vault) and can only
be released via signature from pre-approved Third-Party Arbitrators.
...
2. **GrantStreamWithArbitration** - Enhanced grant streaming with integrated
   dispute resolution
...
- **Flexible Decisions**: Support for partial refunds, split awards, and full
  resolutions
...
This documentation provides a comprehensive overview of the Dispute Resolution
Arbitration Escrow system. For specific implementation details, refer to the
individual contract documentation and test suites.
