...
This document outlines all security invariants that must never be violated in
the Grant-Stream smart contract system. These invariants serve as the
fundamental security guarantees of the protocol and are essential for auditors
...
**Rule:** The sum of all distributed funds can never exceed the initial grant
amount.
...
**Rule:** All timestamps must be monotonically increasing and within reasonable
bounds.
...
**Rule:** Multi-signature operations must meet required thresholds before
execution.
...
**Rule:** Validator rewards are exactly 5% of all accruals when validator is
set.
...
These invariants represent the fundamental security properties of the
Grant-Stream protocol. Any modification to the contract must preserve all
invariants, and auditors should use this document as the primary reference for
